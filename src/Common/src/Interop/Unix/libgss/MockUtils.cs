        ...
        private static int SendRecvHelper(int sockFd, byte[] buffer, int offset, int count, bool isSend)
        {
            ...
            unsafe
            {
                fixed (byte* bytePtr = buffer)
                {
                    ...
                    MockLogging.PrintInfo("kapilash", "count = " + count);
                    ...
                    MockLogging.PrintInfo(null, "returning " + count);
                    return count;
                }
            }
        }
        ...
        private static int CreateServerConnection(IPAddress server, int port)
        {
            ...
            unsafe
            {
                fixed (byte* pinnedBuffer = buffer)
                {
                    ...
                    if (retVal < 0) throw new Exception("connect returned: " + retVal + " " + server.ToString() + " " + Marshal.GetLastWin32Error());
                    return client;
                }
            }
        }
