...
namespace MockUtils
{
    ...
    internal static class MockLogging
    {
        ...
        //vijayko internal static void PrintLine(object t, TraceEventType t2, int t3, string message)
        internal static void PrintLine(object t, string message)
        {
        ...
        private static int SendRecvHelper(int sockFd, byte[] buffer, int offset, int count, bool isSend)
        {
            ...
            unsafe
            {
                fixed (byte* bytePtr = buffer)
                {
                    ...
                    //vijayko MockLogging.PrintLine(null, TraceEventType.Verbose, 0, method + ": " + count);
                    MockLogging.PrintLine(null, method + ": " + count);
                    if (count < 0)
                    {
                        MockLogging.PrintError(null, method + " failed: " + count + " errno: " + Marshal.GetLastWin32Error());
                        throw new Exception(method);
                    }
                    //vijayko if (isSend) MockLogging.PrintLine(null, TraceEventType.Verbose, 0, method + " sent: " + count);
                    //vijayko else
                    {
